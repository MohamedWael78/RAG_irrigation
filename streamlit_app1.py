"""
streamlit_app.py – AquaMind AI with full Image Reader system.

Features:
  • PDF image extraction + vision-model description → searchable image chunks in ChromaDB
  • "📷 Document Images" sidebar panel showing extracted PDF images with descriptions
  • Enhanced field image upload with preset analysis prompts and detailed results
  • Hybrid BM25+Vector+RRF+FlashRank retrieval (searches text AND image descriptions)
  • Page-based citations with image-type badge for diagram/figure sources
"""
...
# ── Auto-build knowledge base (text + image descriptions) ──
def ensure_knowledge_base():
    ...
    if need_build:
        ...
        # ── Add image descriptions to ChromaDB ──
    ...
    if images_meta and m07.GROQ_API_KEY and not os.path.isfile(desc_file):
        with st.spinner("📷 Describing PDF images ..."):
            ...
            # Descriptions cached – check if they're already in ChromaDB
            try:
                vs = m06.get_vector_store()
                # Simple check: search for an image-type document
                test_results = vs.similarity_search("diagram irrigation schematic", k=1,
                    filter={"type": "image"})
                if not test_results:
                ...
                pass  # Filter might not be supported; descriptions likely already added
...
with st.sidebar:
    ...
    if extracted_images:
        with st.expander(f"📷 View {len(extracted_images)} Extracted PDF Images", expanded=False):
            ...
            for img in extracted_images[:10]:  # Show first 10
    ...
    # ── Describe remaining images button ──
    if extracted_images and m07.GROQ_API_KEY:
        desc_file = os.path.join("data", "images", "image_descriptions.json")
    ...
    if uploaded_image is not None:
        ...
        selected_analysis = st.selectbox(
            "Analysis Type", analysis_presets, key="analysis_type"
        )
...
# ── LLM + Agent ──
@st.cache_resource(show_spinner="جاري تهيئة الـ Agent...")
def get_cached_agent(lat: float, lon: float):
...
try:
    agent_executor = get_cached_agent(field_lat, field_lon)
    llm_available = True
except Exception as e:
    llm_available = False
    st.warning("⚠️ m07.GROQ_API_KEY not configured or failed to initialize.")
...
if "messages" not in st.session_state:
    st.session_state.messages = [{
        ...
        "content": (
            ...
            "I can:\n"
            "- 🔍 Search your PDF knowledge base (text + diagrams/images)\n"
            "- 📷 Analyze field photos you upload\n"
            "- 🧮 Calculate drip irrigation parameters\n"
            "- 🌤️ Estimate ET0 for your location\n"
            "- 🌱 Look up crop coefficients\n\n"
...
input_text = st.chat_input("Ask about irrigation, crops, soil... or upload a field image 📷")
...
if input_text:
    ...
    if st.session_state.image_analysis:
        ...
        # تم حذف unsafe_allow_html لحماية التطبيق من ثغرات XSS
        ...
        # ── Standard message (no image analysis pending) ──
        ...
        with st.chat_message("assistant"):
            try:
                if not llm_available:
                    answer = "⚠️ m07.GROQ_API_KEY not configured."
                    st.markdown(answer)
                    st.session_state.messages.append({
                        "role": "assistant",
                        "content": answer,
                    ...
                    # دالة البث المباشر (Streaming Generator)
                    def generate_stream():
                        for chunk in agent_executor.stream({"input": input_text, "chat_history": chat_history}):
                            # 1. التقاط المراجع من مخرجات خطوات الأدوات (Tools Execution)
                            if "steps" in chunk:
                            ...
                            # 2. إرسال النص المتولد تدريجياً لـ Streamlit
                            if "output" in chunk:
                                yield chunk["output"]

                    # بث الإجابة كلمة بكلمة بدلاً من الانتظار داخل st.spinner
                    answer = st.write_stream(generate_stream())

                    # عرض بطاقات المصادر والمراجع بعد اكتمال البث النصي
                    if all_citations:
                        try:
                ...
                if "tool_call" in err_text.lower():
                    fallback_msg = "⚠️ Tool call formatting issue. Try rephrasing with explicit numbers."
                else:
                    fallback_msg = f"⚠️ Error occurred: {err_text}"
                
                st.error(fallback_msg)
                
                # حل مشكلة State Desync عبر إضافة رد الخطأ للجلسة
                st.session_state.messages.append({
                    "role": "assistant",
